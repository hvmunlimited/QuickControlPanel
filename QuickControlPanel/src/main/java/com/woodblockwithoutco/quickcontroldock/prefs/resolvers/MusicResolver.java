...
public class MusicResolver extends BasePrefsResolver {
	...
	public static PendingIntent getDefaultPlayerIntent(Context context) {
		String playerIntent = getString(context, Keys.Music.DEFAULT_PLAYER, "broadcast");
		if(playerIntent.equals("broadcast")) return null;
		
		return MediaReceiverResolver.getMediaPendingIntent(context, playerIntent);
	}
	...
	public static int getArtworkSize(Context context) {
		int sizeDp = getInt(context, Keys.Music.ARTWORK_SIZE, 120);
		return ScreenUtils.dipToPixels(context, sizeDp);
	}
	
	public static int getArtworkPadding(Context context) {
		int sizeDp = getInt(context, Keys.Music.ARTWORK_PADDING, 8);
		return ScreenUtils.dipToPixels(context, sizeDp);
	}
